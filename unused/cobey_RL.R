...
for (country in c('Canada', 'United States', 'United Kingdom', 'Japan', 'Germany', 'France')){
  ...
  # Deconvolution step
  rls = get_RL(data$new_cases_per_million, c(1:length(data$date)), dist_to_infectious$`real_world`, max_iter=50, regularize=0.1, regularize_type='TV', right_censor=TRUE)
