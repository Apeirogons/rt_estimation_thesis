...
linear_filter = function(incidence, N=7, level=0.99){
  linear_filter_base = function(i){
    ...
    prediction = predict(model, newdata=temp_df, interval='confidence', level=level)
  ...
  filtered[1:N_half,] = predict(model, newdata=left_df, interval='confidence', level=level)
  ...
  filtered[(length(incidence)-N_half+1):(length(incidence)),] = predict(model, newdata=right_df, interval='confidence', level=level)
  return(filtered)
}
