...
plot = ggplot(seir)  + geom_line(aes(x=X, y=true_incidence, color='true incidence')) + scale_color_colorblind()
...
rls = get_RL(wt$`Mean(R)`, wt$mean_t, si, max_iter=200, regularize=0.025, stopping_n=0.01)
rls=rls[rls['time'] >=0,]
